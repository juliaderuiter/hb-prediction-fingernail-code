...
# Configurations --------------------------------------------------------------------------------------------
metadata = # Load benchmark metadata in
image_folder = # Load benchmark image folder
results_folder = # Assign results folder of EDA for benchmark

sanquin_hb_csv = # Load Sanquin data Hb values in
sanquin_image_folder = # Load Sanquin data image folder
sanquin_results_folder = # Assign results folder of EDA for Sanquin data
...
def plot_rgb_percentiles_benchmark(features_per_nail, results_folder):
    ...
    # Histogram of mean intensity per feature per channel
    channels = ["R", "G", "B"]
    colors = {
        "R": "#E24A33",
        "G": "#59A14F",
...
def run_eda_sanquin(hb_csv_path, image_dir, sanquin_results_folder):
    ...
    def count_nails_for_id(id_str):
        count = 0
        for k in range(1, 5):
            filename = f"ID_{id_str}_NAIL_{k}.png"
...
def preprocess_data(metadata, image_folder, *, n_nails=1): 
    ...
    # Density histogram before and after KDE sampling
    if eda_run:
...
# Comparing the features of the benchmark and Sanquin data
